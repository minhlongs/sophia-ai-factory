...
// Expected claims in RaaS Gateway JWT
{
  ...
  // Possibly signed with RAAS_JWT_SECRET=REDACTED
}
```
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext) {
    ...
    const claims = await verifyEnrichedJwt(token, env.JWT_SECRET=REDACTED);
    if (!claims) {
...
export async function createEnrichedJwt(
  ...
  return new SignJWT(payload as EnrichedJwtPayload)
    ...
    .sign(new TextEncoder().encode(process.env.JWT_SECRET=REDACTED));
}
...
export async function verifyEnrichedJwt(
  ...
  try {
    const { payload } = await jwtVerify(
      token,
      new TextEncoder().encode(process.env.JWT_SECRET=REDACTED)
    );
...
JWT_SECRET=REDACTED=<strong-random-secret-min-32-bytes>
RAAS_JWT_SECRET=REDACTED=<separate-secret-for-raas-gateway-jwt>
```
