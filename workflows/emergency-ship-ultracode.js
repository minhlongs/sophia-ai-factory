export const meta = {
  name: 'ship-sophia-emergency',
  description: 'Emergency ship pipeline: preflight→merge→tests→review→version→commit→push→PR',
  phases: [
...
const CWD = '/Users/macbook/projects/sophia-ai-factory';

// Helper: run bash command via agent - returns string
async function bash(cmd, phase, label) {
  const result = await agent(cmd, { phase, label, isolation: 'worktree' });
  return result || '';
}

// Wait for agent to complete - agent returns string directly
async function run(cmd, phase, label) {
  return await agent(cmd, { phase, label, isolation: 'worktree' });
}
...
// Get current branch
const branchRaw = await run('git branch --show-current', 'Pre-flight', 'current-branch');
...
if (!currentBranch.startsWith('feature/') && !currentBranch.startsWith('hotfix/') && !currentBranch.startsWith('bugfix/')) {
  throw new Error(`Branch ${currentBranch} không phải release branch. Dừng pipeline.`);
}

// Check git status
const statusRaw = await run('git status --porcelain', 'Pre-flight', 'git-status');
...
// Diff stats
const diffRaw = await run('git diff --stat HEAD', 'Pre-flight', 'diff-stats');
const diff = diffRaw || '';
log(`📊 Diff: ${diff.split('\n')[0] || 'no changes'}`);
...
log('━');
log('🔄 Fetching origin...');
await run('git fetch origin', 'Merge', 'fetch-origin');
log('✅ Fetched');

log('🔄 Merging origin/main...');
const mergeRaw = await run('git merge origin/main --no-edit', 'Merge', 'merge-main');
...
const testRaw = await run('npm run ci:test', 'Tests', 'full-test-suite');
...
const reviewRaw = await run('npx code-reviewer --level high || true', 'Review', 'critical-review');
...
log('━');
log('📦 Version bump...');
const pkgRaw = await run('cat package.json', 'Version', 'read-pkg');
const pkgStr = pkgRaw || '{}';
...
try {
  ...
  await run(`npm version ${newVersion} --no-git-tag-version`, 'Version', 'apply-bump');
...
const changelogRaw = await run('npx conventional-changelog -p angular -i CHANGELOG.md -s -r 0 || true', 'Changelog', 'generate');
const changelog = changelogRaw || '';
if (changelog.includes('CHANGELOG.md') || (await run('test -f CHANGELOG.md 2>/dev/null && echo "exists"', 'Changelog', 'check-file')).includes('exists')) {
...
await run('git add -A', 'Commit', 'git-add');

const shaRaw = await run('git rev-parse HEAD', 'Commit', 'get-sha');
...
await run(`git commit -m "${commitMsg.replace(/"/g, '\\"')}"`, 'Commit', 'git-commit');
log('✅ Committed');

// PHASE 8: Push
log('━');
log('🚀 Pushing to remote...');
const pushRaw = await run(`git push -u origin ${currentBranch}`, 'Push', 'git-push');
const push = pushRaw || '';
...
const prRaw = await run(`gh pr create --base main --head ${currentBranch} --title "${prTitle}" --body "${prBody.replace(/"/g, '\\"')}"`, 'PR', 'create-pr');
