...
- Only `JWT_SECRET=REDACTED` present; `BETTER_AUTH_SECRET` (required by env-validation.ts) absent
...
2. Is `BETTER_AUTH_SECRET` intentionally replaced by `JWT_SECRET=REDACTED` (custom auth)? Code uses `JWT_SECRET=REDACTED` for custom PBKDF2 JWT — if so, `env-validation.ts` schema should be updated to match.
