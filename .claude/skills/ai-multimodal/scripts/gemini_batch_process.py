...
try:
    ...
    load_dotenv = None  # Not needed when centralized resolver is available
except ImportError:
            ...
            for rotation_attempt in range(max_rotation_attempts):
                ...
                if (result.get('rate_limited') and rotator and
                    ...
                    if new_client:
                        client = new_client
                        if verbose:
                            print("  Retrying with rotated key...")
                        continue
                    else:
                        # All keys exhausted - mark result with clear error
                        if verbose:
                            print("  ⚠ All API keys exhausted (on cooldown)", file=sys.stderr)
