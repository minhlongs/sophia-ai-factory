...
const CLAIM_CHECKS: ClaimCheck[] = [
  ...
  },
  {
    ...
    check: async () =>
      fileExists('app/api/v1/missions/[id]/stream/route.ts') ||
      fileExists('app/api/analytics/realtime/route.ts'),
  },
  {
    claim: 'Telegram bot',
    description: 'Telegram webhook route exists',
    check: async () =>
      fileExists('app/api/webhooks/telegram/route.ts') ||
      fileExists('lib/telegram'),
  },
  {
    claim: 'MCU credits transparent',
    description: 'MCU credits repo exists',
    check: async () =>
      fileExists('lib/mcu/credits-repo.ts') ||
      fileExists('lib/usage-metering'),
  },
  {
  ...
  },
  {
    ...
    check: async () =>
      fileExists('lib/missions/fire-webhook.ts') ||
      fileExists('lib/webhooks'),
  },
  {
    claim: 'NOWPayments integration',
    description: 'NOWPayments IPN webhook route exists',
    check: async () =>
      fileExists('app/api/webhooks/nowpayments/route.ts') ||
      fileExists('app/api/billing'),
  },
]
