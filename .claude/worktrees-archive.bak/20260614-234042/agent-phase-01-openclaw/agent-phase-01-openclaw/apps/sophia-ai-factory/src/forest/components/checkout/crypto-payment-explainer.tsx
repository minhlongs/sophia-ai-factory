    ...
    <div className="rounded-xl border border-border bg-muted/30 overflow-hidden">
      ...
      {open && (
        <div className="border-t border-border px-4 pb-4 pt-3 space-y-4">
          <p className="text-xs text-muted-foreground">{t("crypto_explainer_subtitle")}</p>

          {/* Step-by-step guide */}
          <ol className="space-y-3">
            {STEPS.map(({ titleKey, descKey }, idx) => (
              <li key={titleKey} className="flex gap-3">
                <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-muted text-xs font-bold text-muted-foreground mt-0.5">
                  {idx + 1}
                </span>
                <div>
