...
FUNCTION detectMode(input):
  ...
  IF input matches path pattern (./plans/*, *.md with plan/phase in name):
    RETURN "code"

  # Priority 3: Keyword detection (case-insensitive)
  keywords = lowercase(input)

  IF keywords contains ["fast", "quick", "rapidly", "asap", "nhanh"]:
    RETURN "fast"

  IF keywords contains ["trust me", "auto", "yolo", "just do it", "tin em"]:
    RETURN "auto"

  IF keywords contains ["no test", "skip test", "without test", "không test"]:
    RETURN "no-test"
...
**Review Gates:** Human approval checkpoints between major steps.
...
"/cook plans/260120-auth/implementation_plan.md"
...
"/cook làm nhanh cái login"
→ Mode: fast ("nhanh" keyword - Vietnamese support)
```
