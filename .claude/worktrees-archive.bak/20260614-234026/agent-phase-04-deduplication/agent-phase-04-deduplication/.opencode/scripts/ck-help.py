...
Scans .opencode/commands/ directory to build catalog at runtime.
...
CATEGORY_GUIDES = {
    ...
    "config": {
        ...
        "workflow": [
            ("Global", "Set user prefs in `~/.opencode/.ck.json`"),
            ("Local", "Override per-project in `./.opencode/.ck.json`"),
    ...
    "notifications": {
        ...
        "workflow": [
            ("1. Set env vars", "Add `DISCORD_WEBHOOK_URL` or `TELEGRAM_BOT_TOKEN`+`TELEGRAM_CHAT_ID` to `~/.opencode/.env`"),
            ("2. Add hook", "Add Stop hook to `.opencode/settings.json` (see below)"),
            ("3. Test", "`echo '{\"hook_event_name\":\"Stop\"}' | node .opencode/hooks/notifications/notify.cjs`"),
        ],
        "tip": """Add to settings.json:
```json
"Stop": [{"matcher": "*", "hooks": [{"type": "command", "command": "node .opencode/hooks/notifications/notify.cjs"}]}]
```
Docs: `.opencode/hooks/notifications/docs/`""",
    },
}
...
def discover_commands(commands_dir: Path, prefix: str) -> dict:
    """Scan .opencode/commands/ and build command catalog."""
    commands = {}
    categories = {}
...
def show_config_guide() -> None:
    ...
    print("- Global: `~/.opencode/.ck.json` (user preferences)")
    print("- Local: `./.opencode/.ck.json` (project overrides)")
    print()
    ...
    print("## Quick Start")
    print()
    print("**Global config** (`~/.opencode/.ck.json`) - your preferences:")
    print("```json")
    print('{')
    print('  "locale": {')
    ...
    print("```")
    print()
    print("**Local override** (`./.opencode/.ck.json`) - project-specific:")
    print("```json")
    print('{')
    ...
    print("```bash")
    print("# ~/.opencode/.ck.json - applies everywhere")
    ...
    print("# Local (.opencode/.ck.json): issuePrefix = \"JIRA-\"")
...
def show_coding_level_guide() -> None:
    ...
    print("- Global: `~/.opencode/.ck.json` - personal preference")
    print("- Local: `./.opencode/.ck.json` - project override")
    print()
    print("---")
    print()
    print("## How It Works")
    ...
    print("2. If 0-5, injects guidelines from `.opencode/output-styles/coding-level-*.md`")
    ...
    print("## Customization")
    print()
    print("Guidelines live in `.opencode/output-styles/coding-level-*.md`")
...
def main():
    # Find .opencode/commands directory
    script_path = Path(__file__).resolve()
    claude_dir = script_path.parent.parent  # .opencode/scripts -> .claude
    commands_dir = claude_dir / "commands"

    if not commands_dir.exists():
        print("Error: .opencode/commands/ directory not found.")
        sys.exit(1)
    ...
    if not data["commands"]:
        print("No commands found in .opencode/commands/")
        sys.exit(1)
