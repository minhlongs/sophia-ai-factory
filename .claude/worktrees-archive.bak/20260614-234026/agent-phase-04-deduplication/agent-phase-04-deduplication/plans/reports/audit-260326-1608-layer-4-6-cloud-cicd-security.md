...
- [x] `JWT_SECRET` absence handled gracefully (503 or /status redirect)
...
- [x] Secrets expected via CF Workers secrets (`JWT_SECRET`, `POLAR_API_KEY`, `INTERNAL_API_SECRET`, etc.)
...
- Are all required CF secrets (`JWT_SECRET`, `POLAR_API_KEY`, `POLAR_WEBHOOK_SECRET`, `INTERNAL_API_SECRET`, `CRON_SECRET`) actually set in CF Workers environment? No verification mechanism found.
