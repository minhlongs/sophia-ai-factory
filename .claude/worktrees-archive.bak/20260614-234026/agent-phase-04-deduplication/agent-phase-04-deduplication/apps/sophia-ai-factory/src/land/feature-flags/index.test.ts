...
describe('feature-flags canary helper', () => {
  ...
  it('returns false when flag is disabled', async () => {
    const store: KvStore = new Map([
      ['feature-flag:my-flag', JSON.stringify({ enabled: false, percent: 100 })],
    ])
    injectKv(store)
    const { isEnabled } = await freshImport()
  ...
  it('returns true for enabled flag at 100% without userId', async () => {
    const store: KvStore = new Map([
      ['feature-flag:full-rollout', JSON.stringify({ enabled: true, percent: 100 })],
    ])
    injectKv(store)
    const { isEnabled } = await freshImport()
  ...
  it('splits ~50% across 100 deterministic userIds at percent:50', async () => {
    const store: KvStore = new Map([
      ['feature-flag:half', JSON.stringify({ enabled: true, percent: 50 })],
    ])
    injectKv(store)
    const { isEnabled } = await freshImport()
  ...
  it('returns false and does not throw on corrupt KV JSON', async () => {
    const store: KvStore = new Map([['feature-flag:corrupt', 'NOT_VALID_JSON{']])
    injectKv(store)
    const { isEnabled } = await freshImport()
  ...
  it('returns false when KV value fails Zod schema (wrong shape)', async () => {
    const store: KvStore = new Map([
      ['feature-flag:bad-shape', JSON.stringify({ enabled: 'yes', percent: 'all' })],
    ])
    injectKv(store)
    const { isEnabled } = await freshImport()
  ...
  it('statistical sanity: 1000 userIds at 30% yields 270–330 truthy results', async () => {
    const store: KvStore = new Map([
      ['feature-flag:thirty', JSON.stringify({ enabled: true, percent: 30 })],
    ])
    injectKv(store)
    const { isEnabled } = await freshImport()
