 ...
 * Handles:
 *   - "aes:v1:<payload>" — current versioned format; tries current key, then prev key (read-repair)
 *   - "aes:<payload>"   — legacy unversioned format (same algorithm, tries same keys)
 *   - "enc:<base64>"    — legacy base64-only format (migration window)
 *   - no prefix         — plaintext pre-encryption
 *
 * Key rotation: set OAUTH_TOKEN_ENC_KEY_PREV to old key. If current key fails and prev key
 * succeeds, returns the decrypted value. The caller is responsible for re-encryption (read-repair)
 * via the re-encrypt helper below if needed.
 *
...
export async function decryptToken(ciphertext: string): Promise<string> {
  ...
  // Determine payload and handle versioned vs legacy aes: prefix
  let payload: string;
  if (ciphertext.startsWith(AES_V1_PREFIX)) {
    ...
    // Legacy unversioned "aes:" — same algorithm, fall through to same decryption
  ...
  // Try previous key if configured (key rotation fallback)
  ...
  if (prevRawKey) {
    try {
      ...
      if (prevResult !== null) {
        // Decrypted with previous key — caller should re-encrypt with current key (read-repair)
        return prevResult;
      }
    } catch {
 ...
 * Use for read-repair after decryptToken succeeds with OAUTH_TOKEN_ENC_KEY_PREV.
 * Callers should call this and update the DB row when they detect the previous key was used.
 */
