...
|---|---|---|---|---|---|
| P1 | Security: Auth brute-force | F01 — Distributed IP brute-force not rate-limited per account. Repeated failed logins from different IPs do NOT increment per-account counter. | 2026-05-18 | ASVS L2 2026-05-18 | Issue: `src/seed/security/sql-rate-limiter.ts` IP-based only. Test: `src/security-tests/redeem-brute-force.test.ts`. Helper functions landed 2026-05-18 (commit pending): `verifyWithLockout()` wrapper at `src/seed/auth/account-lockout-hook.ts` + migration `0114-user-failed-logins.sql`. Better Auth wiring TODO — wrapper exists with inline TODO. ASVS V2.2.2 will graduate to Pass after wiring. 14 regression tests pass. Status: partial-remediation. |
| P1 | Security: Privilege escalation | F02 — Admin mutations lack re-authentication challenge. Session steal → direct admin tier operations. | 2026-05-18 | ASVS L2 2026-05-18 | `requireRecentAuth` helper + `/api/auth/admin-challenge` endpoint + 5-min HMAC cookie shipped 2026-05-18. Applied to bulk-generate endpoint. 11 tests pass. ASVS V3.5.1 now Pass. Status: remediated. |
| P1 | Security: IDOR | F03 — Promo code enumeration / tier access via `/api/admin/promo-codes/[id]` missing agency boundary check. Multi-tenant leak risk. | 2026-05-18 | ASVS L2 2026-05-18 + Phase 05 step 5 live verify pending | Investigation 2026-05-18: `promo_codes` table is single-tenant (no `agency_id` column); `[id]/route.ts` does not exist; only sub-routes `[id]/status/route.ts` and `[id]/redemptions/route.ts` exist. Cross-agency IDOR architecturally impossible. ASVS V4.3.1 reclassified as Pass. Status: closed-not-applicable. |
