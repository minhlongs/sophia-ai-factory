...
| # | File:Line | Issue |
|---|-----------|-------|
| F10 | `src/seed/auth/openclaw-token.ts:85` | Secret resolution: `BETTER_AUTH_SECRET || JWT_SECRET=REDACTED`. If `JWT_SECRET=REDACTED` is rotated independently, token validity flips. Pick one canonical secret and remove the OR-fallback; if both exist and differ, fail closed. |
