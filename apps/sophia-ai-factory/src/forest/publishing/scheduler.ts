...
import { getRnnScheduler, type OptimalTimeResult } from './rnn-scheduler';
...
export async function schedulePublish(input: SchedulePublishInput): Promise<SchedulePublishResult> {
 ...
 // RNN enhancement: try engagement-based optimal time for higher confidence.
 // Non-blocking — falls back to the generic golden-hour slot above on any error.
 try {
   const rnn = getRnnScheduler();
   const rnnResult = await rnn.getOptimalPublishTime(
     channel.provider as ChannelProvider,
     videoJobId,
   );
   if (rnnResult.time && rnnResult.confidence >= 0.55) {
     const rnnUnix = Math.floor(rnnResult.time.getTime() / 1000);
     if (rnnUnix !== effectiveScheduledAt) {
       logger.info('[Scheduler] RNN override applied', {
         channel: channel.provider,
         oldTime: effectiveScheduledAt,
         newTime: rnnUnix,
         confidence: rnnResult.confidence,
         source: rnnResult.source,
       });
       effectiveScheduledAt = rnnUnix;
     }
   }
 } catch (err) {
   // RNN is a soft enhancement — never break the scheduling flow.
   logger.warn('[Scheduler] RNN hook skipped (non-fatal)', {
     channel: channel.provider,
