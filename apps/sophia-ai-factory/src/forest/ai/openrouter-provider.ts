...
import { verifyFinalContext, compressIfNeeded } from '@/seed/ai/context-window-enforcer';
...
export class OpenRouterProvider implements Provider {
  ...
  async chat(messages: ChatMessage[], options: ChatOptions): Promise<ChatResponse> {
    ...
    const managedMessages = compressIfNeeded(messages, model);
    verifyFinalContext(managedMessages, model);

    const body = this.buildRequestBody(managedMessages, options, false);
  ...
  async *stream(
    ...
    const body = this.buildRequestBody(managedMessages, options, true);
