...
export class CostAwareRouter {
  ...
  async chat(prompt: string, messages: ChatMessage[], options: ChatOptions): Promise<RoutedChatResult> {
    ...
    if (this.enforceCostLimits && costDecision.estimatedCost > tierLimit) {
      ...
      if (cheaperProvider) {
        ...
        return this.router.chat(prompt, messages, adjustedOptions);
      }
    }
  ...
  private selectCostAwareProvider(
    chain: Array<{ id: ProviderId; provider: { estimateCost: (messages: ChatMessage[], model: string, options?: ChatOptions) => number; getCapabilities: (model: string) => { maxOutputTokens: number } }; health: { avgLatencyMs: number } }>,
    messages: ChatMessage[],
    options: ChatOptions,
    complexity: Complexity,
    ...
    const estimates = chain.map((entry) => {
      const model = entry.id === routeDecision.provider
    ...
    const usedCheaperAlternative = selected.id !== routeDecision.provider
  ...
  private findCheaperProvider(
    chain: Array<{ id: ProviderId; provider: { estimateCost: (messages: ChatMessage[], model: string, options?: ChatOptions) => number }; health: { avgLatencyMs: number } }>,
    messages: ChatMessage[],
    options: ChatOptions,
    budget: number,
