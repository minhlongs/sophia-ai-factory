 ...
 * against the tier limit defined in forest/quota/video-quota.ts.
 *
 * Returns { allowed, used, limit, resetsAt, reason? } so callers
 * can return 429 with retry-after metadata.
 *
 * Moved from seed/auth/enforce-tier-quota to forest/auth (M4 migration).
 * This module legitimately depends on forest/quota/video-quota, so it
 * belongs in forest, not seed.
 *
