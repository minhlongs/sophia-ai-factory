  ...
  async ({ event, step }) => {
    ...
    try {
      ...
      const videoJobId = await runStepSafely('start-video-generation', async () => {
        ...
        try {
          ...
          void captureServer({
            ...
            logger.warn('[generateCampaign] capture first_video_started failed', { error: String(err) })
          })
          return jobId
        } catch (err) {
          if (err instanceof MissingCredentialsError) {
      ...
      void captureServer({
        ...
        logger.warn('[generateCampaign] capture first_video_completed failed', { error: String(err) })
      })
    ...
    const distributionResult = await runStepSafely('distribute-channels', async () => {
      ...
      await runStepSafely('finalize-campaign', async () => {
        ...
        void captureServer({
          ...
          logger.warn('[generateCampaign] capture campaign_published failed', { error: String(err) })
        })
