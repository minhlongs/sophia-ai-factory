...
interface Env {
  KV_KV: KVNamespace;
  ENVIRONMENT: string;
  JWT_SECRET=REDACTED?: string;  // Secret for JWT signature verification
}
...
export async function verifyJwt(token: string, env: Env): Promise<JWTPayload | null> {
  try {
    ...
    if (env.JWT_SECRET=REDACTED) {
      const signatureValid = await verifyJwtSignature(token, env.JWT_SECRET=REDACTED);
      if (!signatureValid) {
        return null;
      }
    }
