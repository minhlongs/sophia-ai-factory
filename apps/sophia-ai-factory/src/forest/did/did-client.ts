 ...
 *
 * Layer: forest (reusable infrastructure integration)
 *
 * @module forest/did/did-client
 */
 ...
 *
 * @throws DidErrorResponse on non-2xx HTTP response or open circuit breaker.
 */
export async function createDidTalk(apiKey: string, req: DidTalkRequest): Promise<DidTalkResponse> {
  if (!shouldAllowRequest('did')) {
    throw {
      code: 'did_circuit_open',
      status: 503,
      message: 'Circuit breaker open for D-ID — too many failures',
    } as DidErrorResponse;
  }
