...
import type { Publisher, PublishMeta, PublishResult, PublishStatus, MetricsJson } from '@/seed/types/channel-provider';
...
export class BlueskyPublisher implements Publisher {
  ...
  async publish(_videoUrl: string, meta: PublishMeta): Promise<PublishResult> {
    if (isMockMode()) {
      logger.warn('[BlueskyPublisher] Mock mode — BLUESKY_PDS_URL missing');
      return { success: true, externalPostId: `mock_bluesky_${Date.now()}` };
    }
