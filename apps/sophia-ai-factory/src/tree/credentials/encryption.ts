/**
 * Credentials encryption -- thin adapter over BYOK key versioning.
 *
 ...
 *
 * Storage format: v<version>:<iv_b64>:<ciphertext_b64>
 * Legacy rows without v prefix are treated as version 1.
...
async function importKeyByVersion(version: number): Promise<CryptoKey> {
  ...
  throw new Error(`CREDENTIALS_DECRYPT_VERSION_UNSUPPORTED: version ${version} is outside the dual-decrypt window`)
}
