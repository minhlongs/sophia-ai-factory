...
import { AgencyTier } from '@/seed/config/tiers/tier-configs'
...
// ── Agency Invoice Config (Option C: Invoice-Level Routing) ────────────────

/** Configuration attached to a pre-registered agency NOWPayments invoice. */
export interface AgencyInvoiceConfig {
  agencyId: number
  tier: AgencyTier
  monthlyPrice: number
  credits: number
...
// Re-export AgencyTier from canonical location for consumer convenience
export type { AgencyTier } from '@/seed/config/tiers/tier-configs'

/** Invoice-level config map for agency payments. Populated at deploy time. */
export const NOWPAYMENTS_AGENCY_INVOICE_IDS: Record<string, AgencyInvoiceConfig> = {
  // Example — invoice IDs created in NOWPayments dashboard + registered here:
  // 'inv_agency_starter_1': { agencyId: 1, tier: 'starter', monthlyPrice: 500, credits: 100 },
  // 'inv_agency_growth_2': { agencyId: 2, tier: 'growth', monthlyPrice: 1500, credits: 500 },
  // 'inv_agency_enterprise_3': { agencyId: 3, tier: 'enterprise', monthlyPrice: 3000, credits: Infinity },
}

/**
 * Override the agency invoice config map at runtime (test helper).
 * Replaces the entire map — not a merge.
 */
export function setAgencyInvoiceConfigs(
  configs: Record<string, AgencyInvoiceConfig>,
): void {
  Object.keys(NOWPAYMENTS_AGENCY_INVOICE_IDS).forEach(k => {
    delete (NOWPAYMENTS_AGENCY_INVOICE_IDS as Record<string, unknown>)[k]
  })
  Object.assign(NOWPAYMENTS_AGENCY_INVOICE_IDS, configs)
}

/** Look up an agency invoice by NOWPayments invoice ID. Returns null if not registered. */
export function lookupAgencyInvoice(invoiceId: string): AgencyInvoiceConfig | null {
  return NOWPAYMENTS_AGENCY_INVOICE_IDS[invoiceId] ?? null
}
