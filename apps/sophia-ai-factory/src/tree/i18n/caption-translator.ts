 ...
 *
 * @module tree/i18n/caption-translator
 */
...
async function translateWithOpenRouter(
  ...
  const systemPrompt = `You are a professional social media translator. Translate the caption to ${localeName(targetLocale)}. Rules: - Preserve all hashtags (translate hashtag text if appropriate for the locale, keep # prefix) - Preserve emojis - Keep brand names, product names, URLs unchanged - Match the tone: casual for social, professional for LinkedIn - Return ONLY the translated caption text, nothing else`
