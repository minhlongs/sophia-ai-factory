 ...
 * Uses character-based approximation to trim text.
 */
function truncateToTokenBudget(text: string, maxTokens: number): string {
  const currentTokens = tokenCounter.estimateTokens(text).tokens;
  if (currentTokens <= maxTokens) return text;

  const ratio = maxTokens / currentTokens;
  const targetChars = Math.floor(text.length * ratio * 0.9); // 10% buffer
  return text.slice(0, Math.max(targetChars, 20)) + '... [Truncated for context budget]';
}
...
export async function executeCeoContext(
  ...
  switch (intent) {
    case 'list_campaigns': {
      ...
      return {
        intent,
        summary: truncateToTokenBudget(`User has ${campaigns.length} campaigns:\n${lines.join('\n')}`, 30000),
        campaigns,
      };
    }
    ...
    case 'get_campaign_status': {
      ...
      return {
        intent,
        summary: truncateToTokenBudget(`Campaign status:\n${lines.join('\n')}`, 30000),
        campaigns,
      };
    }
    ...
    case 'create_campaign': {
      try {
        ...
        return {
          intent,
          summary: truncateToTokenBudget(result, 30000),
          actionResult: result,
        };
      } catch (err) {
        ...
        return {
          intent,
          summary: truncateToTokenBudget(`Failed to create campaign: ${errMsg}`, 30000),
        };
      }
    }
    ...
    default: {
      return {
        intent,
        summary: truncateToTokenBudget('No specific data context needed. Provide general business analysis.', 30000),
      };
    }
  }
}
