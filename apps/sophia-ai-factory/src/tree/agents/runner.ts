...
import { ConversationSummarizer } from '@/seed/ai/conversation-summarizer';
...
export async function runAgent(taskId: string, orgId: string, userTier = 'BASIC'): Promise<AgentTask> {
  ...
  try {
    ...
    const modelLimit = contextWindow.getLimit(agent.model).contextLimit;
    const systemTokens = tokenCounter.estimateTokens(enrichedPrompt).tokens;
    const userTokens = tokenCounter.estimateTokens(task.input).tokens;
    const totalTokens = systemTokens + userTokens;

    log.info('[runner] Context budget check', {
      model: agent.model,
      limit: modelLimit,
      used: totalTokens,
      utilization: `${((totalTokens / modelLimit) * 100).toFixed(2)}%`
    });

    if (totalTokens > modelLimit * CRITICAL_THRESHOLD) {
      void contextOverflowLog.log({
        provider: 'openrouter',
        tokensUsed: totalTokens,
        limit: modelLimit,
        contextWindow: agent.model,
        agentId: agent.id,
      });
      throw new Error(`Context overflow prevention: Prompt size (${totalTokens} tokens) exceeds critical threshold (${Math.round(modelLimit * CRITICAL_THRESHOLD)} tokens) for model ${agent.model}`);
    }
    // ────────────────────────────────────────────────────────────────────
