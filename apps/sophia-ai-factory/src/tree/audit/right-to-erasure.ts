  ...
  try {
    ...
    for (const log of existingLogs) {
      const { error: updateError } = await db.from<RaasAuditLogRow>('raas_audit_logs')
        .update({
          user_id: 'ANONYMIZED_' + (await generateUserPseudonym(userId)).slice(0, 8),
          ip_address: log.ip_address ? 'ANONYMIZED_' + (await hashIpAddress(log.ip_address)).slice(0, 8) : null,
          user_pseudonym: await generateUserPseudonym(userId),
          ip_address_hash: log.ip_address ? await hashIpAddress(log.ip_address) : null,
        })
        .eq('id', log.id)
