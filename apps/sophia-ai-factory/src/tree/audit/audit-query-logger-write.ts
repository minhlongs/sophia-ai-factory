...
export async function logAuditQuery(params: AuditQueryLogParams): Promise<boolean> {
  ...
  const logData: Record<string, unknown> = {
    ...
    details: { apiKeyId: params.apiKeyId, filters: params.filters, resultCount: params.resultCount, duration: params.duration, timestamp: createdAt },
  }
  try {
    const { data, error } = await insertTyped(db.from('raas_audit_logs'), logData).select().single()
...
export async function logApiKeyCreation(userId: string, apiKeyId: string, permissions: string[], ipAddress?: string): Promise<boolean> {
  ...
  const logData: Record<string, unknown> = {
    ...
    details: { apiKeyId, permissions, timestamp: createdAt },
  }
  try {
    const { error } = await insertTyped(db.from('raas_audit_logs'), logData)
...
export async function logApiKeyRevocation(userId: string, apiKeyId: string, reason?: string, ipAddress?: string): Promise<boolean> {
  ...
  const logData: Record<string, unknown> = {
    ...
    details: { apiKeyId, reason: reason || null, timestamp: createdAt },
  }
  try {
    const { error } = await insertTyped(db.from('raas_audit_logs'), logData)
...
export async function logApiKeyValidationFailure(apiKeyId: string, error: string, ipAddress?: string): Promise<boolean> {
  ...
  const logData: Record<string, unknown> = {
    ...
    details: { apiKeyId, error, timestamp: createdAt },
  }
  try {
    const { error: insertError } = await insertTyped(db.from('raas_audit_logs'), logData)
