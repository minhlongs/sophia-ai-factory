...
describe('GDPR Redaction Module', () => {
  ...
  describe('hashIpAddress', () => {
    it('produces consistent SHA-256 hashes for same IP', async () => {
    ...
    it('handles IPv6 addresses', async () => {
      const hash = await hashIpAddress('2001:0db8:85a3:0000:0000:8a2e:0370:7334')
      expect(hash).toHaveLength(64)
    })

    it('returns empty string for invalid input', async () => {
      expect(await hashIpAddress('')).toBe('')
      expect(await hashIpAddress(null as unknown as string)).toBe('')
    })

    it('produces deterministic hash with salt', async () => {
      // Verify hash is deterministic (same input = same output)
      const hash1 = await hashIpAddress('10.0.0.1')
      const hash2 = await hashIpAddress('10.0.0.1')
      expect(hash1).toBe(hash2)
  ...
  describe('generateUserPseudonym', () => {
    it('produces consistent pseudonym for same user ID', async () => {
      const userId = 'user-123'
      const pseudo1 = await generateUserPseudonym(userId)
      const pseudo2 = await generateUserPseudonym(userId)
      expect(pseudo1).toBe(pseudo2)
      expect(pseudo1).toHaveLength(64)
    })

    it('produces different pseudonyms for different users', async () => {
      const pseudo1 = await generateUserPseudonym('user-1')
      const pseudo2 = await generateUserPseudonym('user-2')
      expect(pseudo1).not.toBe(pseudo2)
    })

    it('returns empty string for invalid input', async () => {
      expect(await generateUserPseudonym('')).toBe('')
      expect(await generateUserPseudonym(null as unknown as string)).toBe('')
    })
  })
  ...
  describe('redactAuditLog', () => {
    ...
    it('pseudonymizes user_id', async () => {
    ...
    it('redacts email in details', async () => {
      const log = createMockLog()
      const redacted = await redactAuditLog(log)
    ...
    it('preserves non-PII details', async () => {
      const log = createMockLog({
    ...
    it('handles null user_id and ip_address', async () => {
  ...
  describe('batchRedactAuditLogs', () => {
    ...
    it('handles large datasets (1000+ logs)', async () => {
    ...
    it('respects redactUserId=false option', async () => {
      const logs = createMockLogs(10)
      const options: RedactionOptions = {
        redactIp: true,
        redactEmail: true,
    ...
    it('handles empty array', async () => {
      const result = await batchRedactAuditLogs([], {
        redactIp: true,
        redactEmail: true,
        redactUserId: true,
      })
