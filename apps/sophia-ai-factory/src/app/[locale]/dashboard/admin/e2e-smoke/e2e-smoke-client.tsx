      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-5 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            {phase !== 'idle' && (
              <span className="ml-3 text-xs text-zinc-500">{elapsed}s</span>
            )}
          </div>
          <div className="flex gap-2">
            {phase === 'idle' || phase === 'completed' || phase === 'failed' ? (
              <button
                onClick={runSmoke}
                className="px-4 py-2 bg-violet-700 hover:bg-violet-600 text-white text-sm rounded-lg font-medium transition-colors"
              >
            ...
            {phase !== 'idle' && (
              <button
                onClick={reset}
                className="px-4 py-2 bg-zinc-700 hover:bg-zinc-600 text-white text-sm rounded-lg font-medium transition-colors"
              >
                {isVi ? 'Đặt lại' : 'Reset'}
              </button>
            )}
        ...
        {result?.success && (
          <div className="rounded-lg bg-zinc-800 p-3 text-xs text-zinc-300 space-y-1">
            <div><span className="text-zinc-500">Payment ID:</span> {result.payment_id}</div>
            <div><span className="text-zinc-500">SKU:</span> {result.sku?.id} — ${result.sku?.priceUsd} / {result.sku?.credits} credits</div>
            <div><span className="text-zinc-500">Target user:</span> {result.target_user_id}</div>
          </div>
        )}
          ...
          <ol className="space-y-2">
            {timeline.map((e, i) => (
              <li key={i} className="flex gap-3 text-xs">
                <span className="text-zinc-600 font-mono shrink-0">{e.ts.split('T')[1]?.slice(0, 8)}</span>
                <span className="text-zinc-300 font-medium shrink-0">{e.phase}</span>
