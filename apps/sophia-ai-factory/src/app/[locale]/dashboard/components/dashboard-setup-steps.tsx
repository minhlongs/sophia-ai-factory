  ...
  return (
    <div className="bg-white/80 dark:bg-slate-900/60 backdrop-blur-md border border-white/20 dark:border-white/10 shadow-lg rounded-xl p-6 space-y-4">
      <div className="text-center mb-6">
        <h2 className="text-xl font-bold text-muted-foreground-900 dark:text-slate-100">{t('setup_title')}</h2>
        <p className="text-sm text-muted-foreground-600 dark:text-slate-400 mt-1">{t('setup_subtitle')}</p>
      </div>
            ...
            <div
              ...
              className={`relative rounded-lg border p-4 transition-colors duration-150 ${
                step.completed
                  ...
                  : 'border-border-200 dark:border-slate-700/50 bg-muted-50/50 dark:bg-slate-800/30'
              }`}
            >
              {step.completed && (
              ...
              <h3 className="text-sm font-semibold text-muted-foreground-900 dark:text-slate-100 mb-1">
                {t(step.titleKey as Parameters<typeof t>[0])}
              </h3>
              <p className="text-xs text-muted-foreground-600 dark:text-slate-400 mb-3">
