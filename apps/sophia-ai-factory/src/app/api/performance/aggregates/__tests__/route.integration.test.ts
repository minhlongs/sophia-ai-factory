/**
 * GET /api/performance/aggregates — Integration Tests
...
// vi.hoisted() for shared mock refs — factories are hoisted by Vitest
const {
  mockDbFirst,
  mockDbBind,
  mockDbAll,
...
describe('GET /api/performance/aggregates', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    denyAccess();
  });
  ...
  it('returns 401 when not authenticated', async () => {
    unauthed();
    const req = new Request('http://localhost/api/performance/aggregates');
  ...
  it('returns 403 when user has no workspace access', async () => {
    authed();
    denyAccess();
    const url = new URL('http://localhost/api/performance/aggregates');
  ...
  it('returns 200 with aggregate data', async () => {
    ...
    const body = await json<{ workspaceId: string; windowHours: number; totalEvents: number; channels: unknown[] }>(res);
