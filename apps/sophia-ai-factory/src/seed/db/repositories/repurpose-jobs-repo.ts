...
const batch = clips.map((clip) => {
const clipId = crypto.randomUUID().replace(/-/g, '').slice(0, 16);
return stmt.bind(clipId, jobId, clip.clipIndex, clip.startMs, clip.endMs, clip.score ?? null, clip.title ?? null);
});
