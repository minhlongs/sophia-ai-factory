...
export async function getUserCredits(
  ...
  const { data } = await db
    ...
    .eq('status', 'paid')
    .gte('expires_at', now)
    // NOTE: .gte('expires_at', now) in D1/Supabase-SDK behaves differently
    // from raw SQLite. In D1 (Cloudflare's SQLite-compatible backend),
    // the Supabase-style `.gte()` filter INCLUDES rows where `expires_at` IS NULL.
    // This is intentional: lifetime credit packs have NULL `expires_at` and
    // should always be treated as "not expired". Do NOT "fix" this to use
    // `expires_at IS NULL OR expires_at > ?` — that would be a no-op in D1.
    // If migrating to a different DB, verify NULL-handling semantics.
