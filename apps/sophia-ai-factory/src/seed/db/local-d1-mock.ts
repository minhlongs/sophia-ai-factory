...
constructor(sqlitePath: string | SqliteDb) {
if (typeof (globalThis as Record<string, unknown>).EdgeRuntime !== 'undefined') {
...
if (typeof sqlitePath === 'string') {
  ...
  sqlitePath.pragma('foreign_keys = OFF');
  this.db = sqlitePath;
}
...
export function getLocalD1Mock(): LocalD1Database | null {
  ...
  let sqlitePath = findLocalD1Path();

  try {
    const Database = eval('require')('better-sqlite3');
    const inMemoryDb = new Database(':memory:');
    if (sqlitePath) {
      logger.info(`[D1 mock] Using wrangler state: ${sqlitePath}`);
    } else {
      logger.info('[D1 mock] No wrangler state found - using in-memory SQLite database');
    }
    return new SQLiteD1Database(inMemoryDb);
  } catch (e) {
