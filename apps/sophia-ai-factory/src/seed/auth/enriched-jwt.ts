...
export async function createEnrichedJwt(
  ...
  try {
    ...
    const payload: EnrichedJwtPayload = {
      ...
      billing_status: dunningState === 'ok' ? 'active' : dunningState === 'grace_period' ? 'past_due' : 'suspended',
      is_paid: dunningState === 'ok' || dunningState === 'grace_period',
      overage_allowed: licenseContext.tier === 'ENTERPRISE' || licenseContext.tier === 'MASTER',
