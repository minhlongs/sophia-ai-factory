...
import { Tier, TierConfig, FeatureFlag } from '@/seed/types';
...
/** Re-export social tier limits from canonical source (tier-social-limits.ts) */
export { TIER_SOCIAL_LIMITS, getSocialTierLimits } from './tier-social-limits';
...
// ── White-Label Agency Tiers ──────────────────────────────────────────────────

export type AgencyTier = 'starter' | 'growth' | 'enterprise';

export const AGENCY_TIERS: Record<AgencyTier, { monthlyPrice: number; maxSubTenants: number; monthlyCredits: number }> = {
  starter: {
    monthlyPrice: 500,
    maxSubTenants: 5,
    monthlyCredits: 100,
