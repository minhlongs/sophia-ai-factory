...
describe('F03 — IDOR /api/admin/promo-codes/[id] (ASVS V4.3.1)', () => {
  ...
  // Regression: verify no flat [id]/route.ts exists (only sub-routes [id]/status, [id]/redemptions)
  // ---------------------------------------------------------------------------
  it('FINDING N-A: [id]/route.ts does not exist — no direct GET/PATCH/DELETE on promo by ID', () => {
    ...
    // [id]/redemptions/route.ts — GET (list redemptions)
    //
    // Regression script: assert the flat [id]/route.ts file is absent.
    // __dirname = <root>/src/security-tests, up 1 → <root>/src
    ...
    // Cross-layer orchestration uses [id]/status and [id]/redemptions sub-routes.
    // These are covered by the admin gate regression tests below.
  });

  // ---------------------------------------------------------------------------
  // FINDING: N-A — promo_codes table has no agency_id (single-tenant design)
  // Regression: verify promo_codes schema has no agency_id / tenant_id
  // ---------------------------------------------------------------------------
  it('FINDING N-A: promo_codes has no agency_id column — multi-tenant IDOR not applicable', () => {
    // promo-codes schema (migration 0066-promo-codes.sql) — global / single-tenant
    // __dirname = <root>/src/security-tests, up 4 → /Users/.../sophia-ai-factory
    const sqlPath = path.join(
      ...
      '..',
      'migrations',
      '0066-promo-codes.sql',
    );
    const sql = fs.readFileSync(sqlPath, 'utf-8');
    expect(sql).not.toMatch(/agency_id/);
    ...
    // TODO 2026-07-15: If multi-tenancy is added, re-evaluate this test —
    // add agency_id column, scope updateCodeStatus/listRedemptionsByCode by
    // session.agency_id, and assert 404 on cross-tenant access.
  });
