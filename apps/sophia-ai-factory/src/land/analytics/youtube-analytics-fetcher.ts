...
export async function fetchYouTubeAnalytics(
  ...
  if (!shouldAllowRequest('youtube-analytics')) {
    throw new Error('[youtube-analytics-fetcher] Circuit breaker open — too many failures');
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      recordFailure('youtube-analytics', kind);
