...
export class BlueskyPublisher implements Publisher {
  ...
  async publish(videoUrl: string, meta: PublishMeta): Promise<PublishResult> {
    try {
      const rkey = await this.doPublish(videoUrl, meta);
      return { success: true, externalPostId: rkey };
    } catch (err) {
      return { success: false, error: err instanceof Error ? err.message : 'unknown' };
    }
  }

  private async doPublish(_videoUrl: string, meta: PublishMeta): Promise<string> {
    if (isMockMode()) {
    ...
    if (!res.ok) {
      const body = await res.text().catch((err) => {
        logger.warn('Failed to read Bluesky createRecord response', { error: String(err), context: 'BlueskyPublisher.publish' });
        return '';
      });
...
/** Create an AT Protocol session via app-password. */
