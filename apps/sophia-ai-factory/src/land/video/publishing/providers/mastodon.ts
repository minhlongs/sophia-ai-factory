...
export class MastodonPublisher implements Publisher {
  ...
  async publish(videoUrl: string, meta: PublishMeta): Promise<PublishResult> {
    try {
      const statusId = await this.doPublish(videoUrl, meta);
      return { success: true, externalPostId: statusId };
    } catch (err) {
  ...
  private async doPublish(videoUrl: string, meta: PublishMeta): Promise<string> {
    ...
    if (!res.ok) {
      const body = await res.text().catch((err) => {
        logger.warn('Failed to read Mastodon status response', { error: String(err), context: 'MastodonPublisher.publish' });
        return '';
      });
  ...
  async delete(postId: string): Promise<void> {
    logger.warn("[mastodon.ts] delete not implemented", { postId });
  }
}
