...
export class ThreadsPublisher implements Publisher {
  ...
  private async doPublish(videoUrl: string, meta: PublishMeta): Promise<string> {
    ...
    if (!containerRes.ok) {
      const body = await containerRes.text().catch((err) => {
        logger.warn('Failed to read Threads container response', { error: String(err), context: 'ThreadsPublisher.publish' });
        return '';
      });
    ...
    if (!publishRes.ok) {
      const body = await publishRes.text().catch((err) => {
        logger.warn('Failed to read Threads publish response', { error: String(err), context: 'ThreadsPublisher.publish' });
        return '';
      });
  ...
  async delete(postId: string): Promise<void> {
    logger.warn("[threads.ts] delete not implemented", { postId });
  }
