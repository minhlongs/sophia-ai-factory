...
describe('computeNext() — parallel_group: same-phase steps', () => {
 ...
 it('should execute both independent parallel-group-1 steps before phase 2', () => {
 const wf = newWorkflow()
 const missions = [
 newMissionWithParallel(1, 'completed', 1, []),
 newMissionWithParallel(2, 'queued', 1, []),
 ]
 ...
 it('should handle 8-step ceo-solo-media workflow lifecycle', () => {
  ...
  const missions: StepMissionRow[] = [
    ...
    { id: 'mission-2', org_id: 'org-1', parent_mission_id: 'wf-1', status: 'queued', params: JSON.stringify({ step_order: 2, step_type: 'audience_analysis', workflow_id: 'wf-1', parallel_group: 1, depends_on: [] }), created_at: now },
    ...
    { id: 'mission-5', org_id: 'org-1', parent_mission_id: 'wf-1', status: 'blocked', params: JSON.stringify({ step_order: 5, step_type: 'content_production', workflow_id: 'wf-1', parallel_group: 3, depends_on: [3, 4] }), created_at: now },
...
// Step 5 waits for both phase-2 deps, so step 4 executes before phase 3 opens.
action = computeNext(wf, missions)
expect(action.action).toBe('execute')
missions[3].status = 'completed'

// After step 4 completes, steps 5 and 6 can both unblock.
action = computeNext(wf, missions)
