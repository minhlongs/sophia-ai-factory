  ...
  try {
    ...
    if (parsed.data.status !== undefined) {
      ...
      if (currentStatus !== newStatus) {
        // Allowed transitions:
        // draft -> published | draft -> archived | draft -> pending_review
        // published -> archived
        // pending_review -> published | pending_review -> draft (admin only)
        // Blocked: published -> draft | archived -> draft | archived -> published
        const allowedFromDraft: Set<string> = new Set(['published', 'archived', 'pending_review']);
        const allowedFromPublished: Set<string> = new Set(['archived']);
        // pending_review transitions are handled in admin functions (approveListingAdmin, rejectListingAdmin)

        const isValid =
          (currentStatus === 'draft' && allowedFromDraft.has(newStatus)) ||
          (currentStatus === 'published' && allowedFromPublished.has(newStatus)) ||
          // Admins will handle pending_review -> published/draft transitions
          false;
        ...
        // When submitting for review from draft, verify the creator profile is active
        if (currentStatus === 'draft' && newStatus === 'pending_review' && profile.status !== 'active') {
          return failure({
            code: 'PROFILE_NOT_ACTIVE',
