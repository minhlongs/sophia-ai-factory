...
if [ "${ALLOW_UNPUSHED_DEPLOY:-0}" != "1" ]; then
  ...
  STATUS_PORCELAIN=$(git -C "$REPO_ROOT" status --porcelain 2>>"$DEPLOY_LOG" | grep -vE "^[?m? ][?MD ] \.cleo(/)?$|^[?m? ][?MD ] \.claude/worktrees(/|\.bak/)|^ M OpenMontage$|^\?\? src/seed/$" 2>>"$DEPLOY_LOG") || true
  ...
  UNTRACKED=$(git -C "$REPO_ROOT" ls-files --others --exclude-standard 2>>"$DEPLOY_LOG" | grep -vE '^\.cleo(/)?$|^\.claude/worktrees(/|\.bak/)|^src/seed/$|^OpenMontage$' 2>>"$DEPLOY_LOG") || true
  if [ -n "$UNTRACKED" ]; then
...
node scripts/inject-interop-shim.mjs 2> >(tee -a "$DEPLOY_LOG" >&2)
...
# ─── Step 3.6: Strip node_modules from .open-next/ (CRITICAL for Workers) ───
# Strips @apm-js-collab and @sentry/* from .open-next/server-functions/default/node_modules/
# These bypass --post-opennext which only operates on .next/ chunks.
echo "==> strip-ssr-bloat (opennext-node-modules)"
bash scripts/strip-ssr-bloat.sh --opennext-node-modules 2>>"$DEPLOY_LOG"
