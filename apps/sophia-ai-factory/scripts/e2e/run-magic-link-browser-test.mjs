...
async function run() {
  ...
  try {
    ...
    // Wait for POST → redirect to /dashboard/onboarding (window.location.href is set by handleGetStarted)
    try {
      await page.waitForFunction(
      ...
      // Not on dashboard/onboarding — capture final URL for diagnosis
      finalUrl = page.url();
      console.error(`[E2E] Redirect did NOT land on /dashboard/onboarding — final URL: ${finalUrl}`);
    }
    ...
    // Check for wizard form selector (WizardClient renders with data-testid="setup-wizard-root")
    const hasWizardRoot = await page.$('[data-testid="setup-wizard-root"]') !== null;
    const hasWizardFallback = await page.$('h1, [class*="wizard"]') !== null;

    const onDashboardOnboarding = finalUrl.includes('/dashboard/onboarding');
    const hasSessionCookie = Boolean(sessionCookie);
    const wizardRendered = hasWizardRoot || hasWizardFallback;

    const verdict = onDashboardOnboarding && hasSessionCookie && wizardRendered ? 'PASS' : 'FAIL';
