...
function main() {
  ...
  const anchor = "async function routingHandler(event";
  if (!code.includes(anchor)) {
    console.error(
      `[inject-interop-shim] Cannot find anchor "${anchor}" in middleware handler.`
    );
    process.exit(1);
  }

  const patched = code.replace(anchor, SHIM + "async function routingHandler(event");
