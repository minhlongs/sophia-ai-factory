...
### Scenario 4: Secret Compromise (API Key / JWT_SECRET=REDACTED Leaked)
...
2. **Rotate immediately:**
   ```bash
   # Generate new secret (example: new JWT_SECRET=REDACTED)
   node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"

   # Update CF Workers secret
   npx wrangler secret put JWT_SECRET=REDACTED
   # Paste new secret, press Ctrl+D
   ```
3. **If JWT_SECRET=REDACTED compromised:**
