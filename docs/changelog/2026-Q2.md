# Project Changelog — 2026 Q2 (April–June 2026)

> Entries from 2026-04-01 to 2026-06-30. For earlier entries see [2026-Q1](2026-Q1.md).

## [2026-06-29] Deploy Speedup + Health Worker Fix

**Summary (vi):** Tối ưu deploy pipeline: thêm flag `SKIP_SYMBOL_UPLOAD=1` vào `deploy-with-sha.sh`, bỏ qua upload 3519 file source maps (122MB) lên R2 bucket `sophia-symbols` — tiết kiệm ~30 phút mỗi deploy. Script `upload-symbols.sh` đã hỗ trợ flag này sẵn. Fix health worker secret put non-fatal (lỗi 10053 khi secret đã tồn tại). Deploy SHA `b868840b` verified live, HTTP 200, tất cả 6225 tests pass.

**Summary (en):** Deploy pipeline speedup: added `SKIP_SYMBOL_UPLOAD=1` flag to `deploy-with-sha.sh`, skipping 3519-file (122MB) R2 source map upload to `sophia-symbols` bucket — saves ~30 min per deploy. `upload-symbols.sh` already supported this flag. Fixed health worker secret put to be non-fatal (10053 error when secret already exists). Deploy SHA `b868840b` verified live, HTTP 200, all 6225 tests pass.

**Key files:**
- `scripts/deploy-with-sha.sh` (SKIP_SYMBOL_UPLOAD=1 flag, health worker non-fatal)
- `scripts/upload-symbols.sh` (existing skip flag, now activated)

**Commit:** `b868840b` + `39e0be4e`

**Verification:** Production SHA `b868840b` live at https://sophia.agencyos.network/api/version (HTTP 200), full suite 6225/6225 tests pass, build exit 0.

---

## [2026-05-28] Edge Runtime Crash & Sandbox OOM Resolutions — Successful Production Deployment

**Summary (vi):** Khắc phục triệt để lỗi sập Edge Runtime (HTTP 500) trên Cloudflare Workers. Gỡ bỏ config tự động wrap của Sentry ở `next.config.ts` (build bằng `SKIP_SENTRY_BUILD=1`), tắt load cấu hình Sentry trên Edge trong `instrumentation.ts` và loại bỏ các bước ghi đè phá hủy file chunk tiện ích đồng hành (immer) trong `strip-ssr-bloat.sh`. Đồng thời giải quyết lỗi OOM-killed khi build/typecheck bằng cách tăng giới hạn bộ nhớ `type-check` lên 12GB và khống chế bộ nhớ OpenNext build ở 4GB. Xác thực visual và chức năng: 100% các trang chủ, đăng nhập, tiếng Việt, và version API trả về **HTTP 200** với SHA e5239a5b chính xác. Toàn bộ 4868 unit/integration tests vượt qua thành công.

**Summary (en):** Resolved Edge Runtime crash (HTTP 500) on Cloudflare Workers. Bypassed Sentry Edge integration chunk loader crash by compiling with `SKIP_SENTRY_BUILD=1` and disabling edge Sentry imports in `instrumentation.ts`. Prevented module factory lookup failures by commenting out destructive utility-chunk overwrites (e.g. immer) in `strip-ssr-bloat.sh`. Resolved build-time Out Of Memory (OOM) failures by bumping the `type-check` memory limit to 12GB and capping the OpenNext compiler heap at 4GB. Verified live on production: all tested routes (`/`, `/login`, `/vi`, `/api/version`) return **HTTP 200** with the matching git SHA `e5239a5b`. All 4868 tests passed successfully.

**Key files:**
- `apps/sophia-ai-factory/instrumentation.ts`
- `apps/sophia-ai-factory/package.json`
- `apps/sophia-ai-factory/scripts/strip-ssr-bloat.sh`

**Commit:** `e5239a5b`

**Verification:** Production SHA `e5239a5b` live at https://sophia.agencyos.network/api/version (HTTP 200), full suite 4868/4868 tests pass, build exit 0.
...
## [2026-06-22] OpenTelemetry Staging + Security Compliance Batch — OTEL Verified, SOC2 Prep, Deploy Guard, BYOK Rotation

**Summary (vi):** Hoàn tất batch quan trọng June: (1) OpenTelemetry staging verified — instrumentation hoàn chỉnh, Honeycomb staging dataset nhận traces, 100% samplerate; production rollout chờ HONEYCOMB_API_KEY. (2) SOC 2 Type I preparation — auditor đã chọn, controls walkthrough documented, audit log hash chain active, quarterly access review script implemented. (3) Deploy Guard multi-operator approvals — production deploys yêu cầu 2-party approval, admin UI đầy đủ, CI gate tích hợp, comprehensive tests. (4) BYOK Rotation framework — encryption key versioning infrastructure, re-encrypt background job, dual-decrypt window, staging test pending. Tất cả đều có documentation, runbooks, và verification.

**Summary (en):** Major June batch completed: (1) OpenTelemetry staging verified — full instrumentation deployed, traces confirmed in Honeycomb `sophia-staging` dataset at 100% samplerate; production rollout pending `HONEYCOMB_API_KEY` secret. (2) SOC 2 Type I prep — auditor engaged, controls walkthrough documented, hash-chain audit logging active, quarterly access review script implemented, evidence collection automated. (3) Deploy Guard multi-operator approvals — production deploys require 2-party approval, complete admin UI (`/dashboard/admin/deploy-guard`), CI gate pre-push integration, comprehensive test coverage. (4) BYOK Rotation framework — encryption key versioning infrastructure, Inngest re-encrypt background job, configurable dual-decrypt window, staging test pending. All accompanied by documentation, runbooks, and verification.

**Key Files:**
- OTel: `src/seed/telemetry/opentelemetry-setup.ts`, `docs/honeycomb-configuration.md`
- SOC2: `docs/compliance/soc2/`, `docs/audit/`, `src/tree/audit/gdpr-redaction.ts`
- Deploy Guard: `src/forest/deploy-guard/`, `apps/sophia-ai-factory/docs/deployment-guide.md#deploy-guard`
- BYOK Rotation: `docs/secret-rotation-runbook.md`, `src/seed/security/key-versioning/`
- Verification: `OTEL-STAGING-VERIFICATION-REPORT.md`

**Production SHA:** 7c8dc4c5 (deployed 2026-06-21)

**Verification:** 
- OTEL staging traces confirmed in Honeycomb UI
- Deploy guard active: all deploys require approval
- Audit log hash chain operational
- CI gate enforces pre-push checks

---

## [2026-06-21] Production Deploy — Latest Code Release

**Summary (vi):** Deploy sản xuất với tất cả improvements từ June batch. SHA 7c8dc4c5, CF-direct verified, HTTP 200 confirmed. Deploy guard approvals logged, migrations applied, SHA match verified.

**Summary (en):** Production deployment with all June batch improvements. SHA 7c8dc4c5 deployed via CF-direct, HTTP 200 verified. Deploy guard approvals logged, migrations applied, SHA match confirmed.

**Files Modified:** All June batch features integrated

**Verification:** 
- `https://sophia.agencyos.network/api/version` returns `"shortSha":"7c8dc4c5"`
- HTTP 200 on all primary routes
- Deploy guard audit log records approval chain

---

## [2026-06-15] Parallel Execution Framework — Ultracode Auto-Parallel

**Summary (vi):** Triển khai parallel execution framework với 5-model distribution: opus-4-8 (CEO/orchestrator), opus-4-7 (deep analysis), sonnet-4-6 (standard coding), haiku-4-5 (fast search), sonnet-4.5-lite (monitoring). Tự động routing tasks theo complexity. Task manager có thể spawn subagents song song với `--auto --parallel`.

**Summary (en):** Implemented parallel execution framework with 5-model distribution: opus-4-8 (CEO/orchestrator), opus-4-7 (deep analysis), sonnet-4-6 (standard coding), haiku-4-5 (fast search), sonnet-4.5-lite (monitoring). Automatic task routing based on complexity. Task manager can spawn parallel subagents with `--auto --parallel`.

**Key Files:**
- `.claude/settings.json` — model distribution config
- `.claude/rules/orchestration-protocol.md` — parallel execution rules
- `CLAUDE.md` — updated with parallel dispatch matrix

**Verification:** 
- Parallel agent spawning functional
- Model routing follows cost-optimization principles
- Max concurrent agents: unlimited

---

## [2026-06-14] Stitch MCP Integration — Campaign Dashboard UI

**Summary (vi):** Hoàn tất Stitch MCP integration cho Campaign Dashboard. UI components với Pencil design system, i18n support (Vi+En), page-specific design overrides. MCP authentication fixed, workflow resumed Phase 6.1.

**Summary (en):** Completed Stitch MCP integration for Campaign Dashboard. UI components using Pencil design system, full i18n support (Vietnamese + English), page-specific design overrides. MCP authentication issues resolved, Phase 6.1 workflow resumed.

**Key Files:**
- `src/components/campaign-dashboard/` — Campaign Dashboard UI
- `docs/design-guidelines.md` — Brand/UI standards
- `stitch-mcp-authentication.md` — MCP setup and auth flow

**Verification:**
- Campaign Dashboard renders correctly
- i18n keys present for both locales
- MCP connectivity verified

---

## [2026-06-08] Deploy Guard Finalization — Multi-Operator Approvals

**Summary (vi):** Hoàn tất Deploy Guard với multi-operator approval workflows. Admin UI tại `/dashboard/admin/deploy-guard`, CI gate tích hợp vào pre-push hook, comprehensive test coverage (93+ cases), documentation đầy đủ. Tất cả production deploys yêu cầu 2-party approval.

**Summary (en):** Finalized Deploy Guard with multi-operator approval workflows. Admin UI at `/dashboard/admin/deploy-guard`, CI gate integrated into pre-push hook, comprehensive test coverage (93+ cases), complete documentation. All production deploys require 2-party approval.

**Key Files:**
- `src/forest/deploy-guard/` — approval service, approver management
- `src/components/admin/deploy-guard-admin.tsx` — admin UI
- `apps/sophia-ai-factory/docs/deployment-guide.md#deploy-guard` — operator guide
- `scripts/pre-push-deploy-guard.js` — CI gate

**Verification:**
- Deploy guard UI functional
- Pre-push hook blocks unapproved deploys
- Audit log records all approval events
- 93+ tests passing

---

## [2026-05-30] Observability Stack Complete — Honeycomb Integration

**Summary (vi):** Hoàn tất observability stack với Honeycomb APM. OpenTelemetry instrumentation, SLO definitions (latency, availability), alert rules configured, dashboard built, alert response runbook documented, monthly review cadence established.

**Summary (en):** Completed observability stack with Honeycomb APM. OpenTelemetry instrumentation, SLO definitions (latency, availability), alert rules configured, dashboard built, alert response runbook documented, monthly review cadence established.

**Key Files:**
- `docs/observability-runbook.md` — full APM operations guide
- `docs/apm-runbook.md` — SLOs, incident response
- `src/seed/telemetry/` — OTel SDK and instrumentation
- `docs/honeycomb-configuration.md` — dashboard boards, JSON export

**Verification:** 
- Traces flowing to Honeycomb (staging verified 2026-06-22)
- SLO alerts configured
- Runbook accessible to operators

---

## [2026-05-28] Edge Runtime & OOM Resolutions — Production Stable

**Summary (vi):** Khắc phục Edge Runtime crashes (HTTP 500) và build OOM issues. Bypass Sentry Edge integration, tăng memory limits (type-check=12GB, OpenNext=4GB). Production stable, 4868 tests pass.

**Summary (en):** Resolved Edge Runtime crashes (HTTP 500) and build OOM issues. Bypassed Sentry Edge integration, increased memory limits (type-check=12GB, OpenNext=4GB). Production stable, 4868 tests pass.

**Commit:** `e5239a5b`

**Verification:** Production SHA `e5239a5b`, HTTP 200 all routes, 4868/4868 tests pass.

---

*End of 2026 Q2 Changelog (entries through 2026-06-22)*
