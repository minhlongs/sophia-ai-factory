...
|-------|--------|-----------|---------|
| **CICD: CI/CD Pipeline Conversion (CF-direct → GitHub Actions)** | ✅ COMPLETE | 2026-09-22 | Converted deployment doctrine from manual local deployment (`deploy-with-sha.sh` / `npm run deploy:full`) to automated GitHub Actions CI/CD (`.github/workflows/deploy.yml` 4-stage pipeline). Local deploy blocked unless `EMERGENCY_CF_DIRECT=1`. Synchronized 13 documentation files, roadmaps, and changelogs. R1–R4 verifications, break-glass test, rollback procedures verified. |
...
| **E3: OpenTelemetry Observability** | ✅ COMPLETE | 2026-09-21 | Production instrumentation wired: `src/instrumentation.ts` Next.js 16 hook dynamically imports `initializeOTel()` (seed layer) → OTLP HTTP trace/metric exporters, `TraceIdRatioBasedSampler` at 1% (`OTEL_SAMPLERATE=0.01`), fetch auto-instrumentation, Honeycomb headers (`x-honeycomb-api-key`, `x-honeycomb-dataset`). Dual-path config (BYOK `platform_configs` table first, env var fallback), graceful no-op when key missing — non-fatal `try/catch` ensures OTEL never blocks cold start. Honeycomb secrets live in CF dashboard (`HONEYCOMB_DATASET=sophia-prod`, `OTEL_EXPORTER_OTLP_ENDPOINT=https://api.honeycomb.io`). Admin endpoints: `/api/admin/honeycomb/test` (key probe), `/api/admin/platform-config` (read/write Honeycomb keys). Deployed to production at SHA `681a53df`. |
| **E4: BYOK Key Rotation** | ✅ COMPLETE | 2026-09-21 | Full E4 ship: `keyRotationCron` + `keyRotationReencrypt` wired into Inngest serve() array, admin UI at `/dashboard/admin/byok-rotation` (page + client component + tests), sidebar nav link, ~45 bilingual i18n keys. Deployed to production at SHA `63753ab2`. 6 commits total (feat + 5 fix commits for Zod v4, Next.js 16 revalidateTag, test mocks). |
